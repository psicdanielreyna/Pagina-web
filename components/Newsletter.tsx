...
type Props = { variant?: "banner" | "card" };
...
export default function Newsletter({ variant = "banner" }: Props) {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    if (pending) return; // evita doble click
    setPending(true);
    setMsg("");

    const fd = new FormData(e.currentTarget);
    if (fd.get("company")) { setPending(false); return; } // honeypot
    try {
      ...
      // Mensajes
      if (res.status === 429) {
        setMsg("Demasiados intentos seguidos. Intenta de nuevo en un momento.");
  ...
  return (
    <section className={["w-full", isBanner ? "py-14 sm:py-16" : "py-8", "bg-[color:var(--nl-bg,#F5F7FA)]"].join(" ")}>
      <div className="mx-auto max-w-5xl px-4">
        <div className={isBanner ? "mb-8" : "mb-5"}>
          ...
          <h2 className={["mt-3 font-semibold tracking-tight text-zinc-900", isBanner ? "text-2xl sm:text-3xl" : "text-xl"].join(" ")}>
        ...
        <div className={["rounded-2xl bg-white p-4 sm:p-6 shadow-sm ring-1 ring-zinc-200", isBanner ? "sm:flex sm:items-center sm:gap-4" : ""].join(" ")}>
          <form onSubmit={onSubmit} className={["flex w-full gap-3", isBanner ? "sm:flex-1" : "", "flex-col sm:flex-row"].join(" ")}>
            <label htmlFor="email" className="sr-only">Correo electrónico</label>
            <input
              id="email" name="email" type="email" required inputMode="email" placeholder="tu@email.com"
          ...
          {isBanner && <div className="mt-3 text-sm text-zinc-500 sm:mt-0">🔒 Prometido: cero spam. Solo contenido útil.</div>}
        </div>

        {!!msg && <p className={`mt-3 text-sm ${/Listo|Ya estabas/.test(msg) ? "text-green-700" : "text-red-700"}`}>{msg}</p>}
      </div>
    </section>
  );
}