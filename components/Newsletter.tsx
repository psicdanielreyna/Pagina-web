  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!res.ok || !data?.ok) {
        ...
        if (data.deferred) {
          setMsg("¡Listo! Si no ves el correo pronto, revisa Spam/Promociones.");
        } else {
  ...
  return (
    <section className={["w-full", isBanner ? "py-14 sm:py-16" : "py-8", "bg-[color:var(--nl-bg,#F5F7FA)]"].join(" ")}>
      <div className="mx-auto max-w-5xl px-4">
        ...
        {msg && (
          <p className={["mt-3 text-sm", /mal|Error|segundos/i.test(msg) ? "text-red-700" : "text-green-700"].join(" ")}>
            {msg}
          </p>
        )}
      </div>
