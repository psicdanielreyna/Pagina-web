...
const posts: Post[] = [
  ...
  },
  {
    ...
    img: "/images/blog/ansiedad-3-pasos.png", // súbela cuando la tengas
    alt: "Post 3",
  },
];
...
export default function HomePage() {
  ...
  return (
    <div className="space-y-16">
      ...
      <section className="py-12 md:py-16">
        <div className="container mx-auto px-4 grid md:grid-cols-2 gap-10 items-center">
          ...
          <div className="relative aspect-[4/3] rounded-2xl overflow-hidden">
            <Image src="/images/header.png" alt="Consulta psicológica" fill className="object-cover" priority />
          </div>
        </div>
      </section>

      {/* RECURSOS DESTACADOS (tamaño tipo Tienda) */}
      <section className="py-8 md:py-12">
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-2xl md:text-3xl font-bold text-slate-900">Recursos destacados</h2>
            <Link href="/tienda" className="text-emerald-700 hover:text-emerald-800 font-medium">Ver todos</Link>
          </div>
            ...
            {recursos.map((r) => (
              ...
              >
                <div className="relative h-56 md:h-64 w-full bg-slate-100">
                  <Image src={r.img} alt={r.alt} fill className="object-cover" sizes="(max-width: 768px) 100vw, 50vw" priority />
                </div>
                <div className="p-5">
                  <h3 className="font-semibold text-slate-900">{r.title}</h3>
                  <p className="mt-1 text-slate-600 text-sm">{r.excerpt}</p>
                  <div className="mt-4">
                    <span className="inline-flex items-center rounded-full border border-slate-200 px-3 py-1 text-sm text-slate-700">Leer más</span>
                  </div>
                </div>
              </Link>
            ))}
      ...
      <section className="py-8 md:py-12">
        <div className="container mx-auto px-4">
          <h2 className="text-2xl md:text-3xl font-bold text-slate-900 mb-6">Último del blog</h2>
          ...
          <div className="grid lg:grid-cols-3 gap-6">
            ...
            >
              <div className="relative h-64 md:h-80 bg-slate-100">
                <Image src={destacado.img} alt={destacado.alt} fill className="object-cover" sizes="(max-width: 1024px) 100vw, 66vw" priority />
              </div>
              <div className="p-5">
                <h3 className="font-semibold text-xl text-slate-900">{destacado.title}</h3>
            ...
            <div className="grid gap-6">
              {resto.slice(0, 2).map((p) => (
                ...
                >
                  <div className="relative h-40 bg-slate-100">
                    <Image src={p.img} alt={p.alt} fill className="object-cover" sizes="(max-width: 1024px) 100vw, 33vw" />
                  </div>
                  <div className="p-4">
              ...
              <Link href="/blog" className="inline-flex items-center justify-center rounded-full px-4 py-2 border border-slate-200 text-slate-700 hover:bg-slate-50">
                Ver blog
              </Link>
            </div>
          </div>
      ...
      {/* NEWSLETTER: ahora es Client Component */}
      <section className="py-8 md:py-12">
        <div className="container mx-auto px-4">
